...
class WaypointUpdater(object):
    ...
    def generate_lane(self):
        '''
        Slice the base waypoints
        '''
        lane = Lane()
        lane.header = self.base_waypoints.header  #This is not used anywhere
        closest_idx = self.get_closest_waypoint_idx() #Get the closest waypoint infront of car
        farthest_idx = closest_idx + LOOKAHEAD_WPS #Consider the next 200 waypoints
        subset_waypoints = self.base_waypoints.waypoints[closest_idx:farthest_idx]

        if self.stopline_wp_idx == -1 or (self.stopline_wp_idx >= farthest_idx):
            lane.waypoints = subset_waypoints  #Just publish as it is and no need of any modification
        else:
            lane.waypoints = self.decelerate_waypoints(subset_waypoints,closest_idx)
    ...
    def decelerate_waypoints(self,waypoints,closest_idx):
        ...
        for i,wp in enumerate(waypoints):
            p = Waypoint()
            p.pose = wp.pose
            # Two waypoints back from the lane so that front of the car stops at the line
            ...
            p.twist.twist.linear.x = min(vel,wp.twist.twist.linear.x) #Kind of trying to maintain the speed limit
            temp.append(p)
        return temp
